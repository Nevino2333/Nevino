/*
 * 音乐播放器视图控制器（共享脚本，全站只加载一次）。
 * 由 MusicPlayer.astro 的轻量引导脚本按需加载并调用 __initMusicWidget(widgetId)。
 * 播放控制委托给 MusicManager（window.__fireflyMusic），视图配置由
 * window.__fireflyMusicViewCfg 提供（MusicManager 注入，避免多实例重复内联）。
 */
(function () {
	function initMusicWidget(widgetId) {

(function () {
    var cfg = window.__fireflyMusicViewCfg;
    ...
    function syncAll() {
        ...
        updatePlayStateUI(s.isPlaying || s.isLoading);
    ...
    on('fm:loading', function (e) {
        var isLoading = e.detail.isLoading;
	...
	window.__initMusicWidget = initMusicWidget;

	function drain() {
		var queue = window.__musicWidgetQueue || [];
		window.__musicWidgetQueue = [];
		for (var i = 0; i < queue.length; i++) {
			try {
				initMusicWidget(queue[i]);
			} catch (e) {
				console.warn("music widget init failed:", e);
			}
		}
	}
