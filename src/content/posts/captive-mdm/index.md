...
1. **网段封锁**：MDM 策略禁止学生平板访问 40 网段（`192.168.40.x`）。这一限制是决定性的——无论超脑热点分配何种 IP，只要落入 40 网段，流量即被平板系统丢弃；
2. **域名解析缺失**：即使网段不被封锁，学生平板也无法将教学域名解析到超脑热点 IP，需要额外的 DNS 配置；
3. **HTTPS 强制**：现代浏览器及 Web 应用普遍强制 HTTPS，本地部署的服务若缺乏有效 TLS 证书，请求将被浏览器直接拦截。
...
1. **连接热点**：学生平板连接超脑开启的 WiFi 热点；
2. **DNS 查询**：平板向超脑 DNS 服务器查询特定域名（如 `spark.changyan.com`）；
3. **劫持应答**：Captive 拦截查询，返回超脑热点 IP；
4. **HTTPS 请求**：平板向该 IP 发起 HTTPS 请求（目标域名仍为 `spark.changyan.com`）；
5. **代理转发**：Captive 监听 443 端口，终止 TLS 加密后将请求转至本地 9001 端口服务；
6. **响应返回**：9001 端口服务的响应经 Captive 加密后返回平板。
...
1. **TLS 终结**：使用本地生成的自签名证书终止 TLS 加密连接；
2. **协议降级**：将解密后的 HTTP 请求转发至 `localhost:9001`；
3. **WebSocket 透传**：识别 `Upgrade: websocket` 请求头，建立 WebSocket 代理隧道，支持实时互动应用；
4. **响应回传**：9001 端口的响应经 Captive 重新加密后返回学生平板。
...
**项目地址**：[https://github.com/ClassIntra/captive](https://github.com/ClassIntra/captive)
