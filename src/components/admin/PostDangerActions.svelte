...
async function submitDelete() {
	...
	if (!window.confirm(`确定永久删除“${draft.title}”吗？此操作不可撤销。`))
		return;
	const online = draft.publicationState !== "draft";
	if (online && !password) return;
	busy = true;
	try {
		const result = await adminRequest<ContentOperation | { deleted: true }>(
			`/drafts/${encodeURIComponent(draft.id)}`,
			{
				method: "DELETE",
				body: JSON.stringify({
					expectedVersion: draft.version,
					...(online ? { password, idempotencyKey: crypto.randomUUID() } : {}),
				}),
			},
		);
		password = "";
		if ("status" in result && isOperationPendingReconciliation(result)) {
			operation = result;
			onnotice("GitHub 已提交删除，数据库状态待对账");
			return;
		}
		onnotice(online ? "删除已提交，等待部署完成" : "草稿已删除");
		ondeleted();
	} catch (cause) {
