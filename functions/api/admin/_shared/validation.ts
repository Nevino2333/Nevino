import { validateContent } from "./markdown-policy";
...
const secretPattern =
	/(ghp_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|AKIA[A-Z0-9]{16}|-----BEGIN [A-Z ]+ PRIVATE KEY-----)/;
...
export const validateDraft = (
	...
	const published =
		stringField(input, "published", limits.published, errors, true) ?? "";
	...
	const sourceLink = stringField(
		input,
		"sourceLink",
		limits.sourceLink,
		errors,
	);
	const licenseName = stringField(
		input,
		"licenseName",
		limits.licenseName,
		errors,
	);
	const licenseUrl = stringField(
		input,
		"licenseUrl",
		limits.licenseUrl,
		errors,
	...
	const content =
		stringField(input, "content", limits.content, errors, true) ?? "";
	...
	if (tagsValue !== undefined) {
		if (
			...
			!tagsValue.every(
				(tag) => typeof tag === "string" && tag.length <= limits.tag,
			)
		)
			errors.push("invalid_tags");
		else tags = tagsValue;
	}
	if (!tags) tags = [];
	...
	for (const field of [
		...
		if (field !== undefined && secretPattern.test(field))
			errors.push("sensitive_value");
	if (image !== undefined && image !== "" && !urlPattern.test(image))
		errors.push("invalid_image");
	if (
		sourceLink !== undefined &&
		sourceLink !== "" &&
		!urlPattern.test(sourceLink)
	)
		errors.push("invalid_sourceLink");
	if (
		licenseUrl !== undefined &&
		licenseUrl !== "" &&
		!urlPattern.test(licenseUrl)
	)
		errors.push("invalid_licenseUrl");
	if (typeof input.pinned !== "undefined" && typeof input.pinned !== "boolean")
		errors.push("invalid_pinned");
	if (
		typeof input.comment !== "undefined" &&
		typeof input.comment !== "boolean"
	)
