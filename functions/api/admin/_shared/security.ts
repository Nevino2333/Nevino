...
const derivePassword = async (password: string, salt: string): Promise<Uint8Array> => {
	const passwordBytes = encoder.encode(password);
	if (passwordBytes.byteLength > MAX_PASSWORD_BYTES) throw new Error("invalid_password");
...
export const hashPassword = async (password: string, salt = randomToken(16)): Promise<string> => {
...
export const verifyPassword = async (password: string, encoded: string): Promise<boolean> => {
	if (password.length > 256 || encoder.encode(password).byteLength > MAX_PASSWORD_BYTES) return false;
	const [scheme, iterations, salt, expected, extra] = encoded.split("$");
	if (scheme !== "pbkdf2" || iterations !== String(PASSWORD_ITERATIONS) || !salt || !expected || extra !== undefined || salt.length > 128 || expected.length > 128) return false;
	try {
		const bits = await derivePassword(password, salt);
...
export const cookie = (name: string, value: string, maxAge: number, secure: boolean): string => `${name}=${value}; Max-Age=${maxAge}; Path=/; HttpOnly;${secure ? " Secure;" : ""} SameSite=Strict`;
...
export const json = (data: unknown, status = 200, headers: HeadersInit = {}): Response => new Response(JSON.stringify(data), { status, headers: { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", ...headers } });
